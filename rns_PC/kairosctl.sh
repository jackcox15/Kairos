#!/usr/bin/env bash
# kairosctl v7.0 - User-Friendly Mesh Operations Console
...
STATUS_SNAPSHOT="$HOME_DIR/kairos-status.txt"
CFG_DIR="$HOME_DIR/.config/kairosctl"
...
WHIPTAIL_TITLE="Kairos Mesh Control"
WHIPTAIL_HEIGHT=24
WHIPTAIL_WIDTH=80
...
ensure_dirs(){
  ...
  [[ -f "$LOG_DIR/kairosctl.log" ]] || touch "$LOG_DIR/kairosctl.log" 2>/dev/null || return 1
...
log(){ 
  printf '%s %s\n' "$(date '+%F %T')" "$*" >>"$LOG_DIR/kairosctl.log" 2>/dev/null || true
}
...
Choose which system to check:" \
  ...
  case "$choice" in
    ...
    2)
      ...
      ;;
    3)
      if [[ -f "$LOG_DIR/kairosctl.log" ]]; then
        text_viewer "System Control Logs" "$LOG_DIR/kairosctl.log"
      else
        error_box "No system control logs found.\n\nLog file: $LOG_DIR/kairosctl.log"
      fi
      ;;
  esac
}
...
generate_snapshot(){
  ...
  {
    echo "KAIROS MESH NODE SNAPSHOT"
    echo "Generated: $(date '+%F %T')"
    echo "Version: kairosctl v${SCRIPT_VERSION}"
    echo "Host: $(hostname)"
    echo "User: $USER"
    echo ""
    echo "================================"
...
# ===== Quick Status (CLI) =====
quick_status(){
  echo "Kairos Mesh Node - Quick Status"
  echo "================================"
  echo ""
...
main(){
  ...
  case "${1:-}" in
    ...
    *)
      ...
      echo "Run 'kairosctl help' for usage information"
      exit 1
      ;;
  esac
}
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  detect_services
  log "kairosctl v${SCRIPT_VERSION} started by $USER"
  main "$@"
fi
